...
- 노출 제품 33개이며 데이터 검수 오류가 있는 행은 공개하지 않음
- 모든 상품에 브랜드·모델·규격을 정리한 검색 링크 제공
- 공식몰 직접 링크는 HTTPS·신뢰 도메인·30일 이내 실응답 확인을 통과해야 함
- 공식몰 확인 실패·만료 시 상품 검색으로 전환하며 쿠팡 직접 링크·이미지 요청은 0개
- 단축 URL 0개
...
가격 신선도 기준은 48시간입니다. 이 시간이 지난 숫자는 최저가로 표시하지 않습니다. 공식몰 링크는 30일 후 만료되며, 이후에도 정리된 상품명 검색은 계속 이용할 수 있습니다. 검색 결과를 판매·재고 확인으로 표시하지 않습니다.
...
네이버 쇼핑 검색 API는 2026년 7월 31일 종료돼 수집 코드와 키 의존성을 제거했습니다. [공식 종료 공지](https://developers.naver.com/notice/article/32530) 현재 자동 가격 수집은 제공하지 않으며, 과거 후보의 확인 날짜를 유지합니다. 신규 가격은 상품·배송비·재고를 검토한 수동 후보로 반영할 수 있습니다.

매일 03:17 KST에 실행되는 `price-candidates.yml`은 공식몰 실응답과 링크 정책을 갱신합니다. 공식몰 일부 실패는 해당 상품만 검색으로 전환합니다. 링크·데이터·테스트·build 검증을 통과한 세 파일만 `main`에 자동 커밋하고 OCI catalog-sync 타이머가 앱 컨테이너를 재배포합니다.

`production-smoke.yml`은 매시 17분과 47분에 운영 URL·정적 파일·HTTPS·보안 헤더와 공식몰 점검 시각(48시간), 전체 상품의 링크 수를 확인합니다. 실패는 GitHub Actions 실행 결과로 남으며 `npm run monitor:production`으로 재현할 수 있습니다.
