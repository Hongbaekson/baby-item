...
| 운영 신뢰 UX | 완료 | 실시간 링크 유효 수치, 검증 원칙, 고정 URL 공유, 상품별 오류 신고, 개인정보 안내 |
...
| 운영 smoke | 완료 | 공개 URL·정적 자산·HTTPS 전환·보안 헤더를 시간당 2회 확인 |
...
- 운영 상태의 유효 링크 수는 화면을 여는 시점의 30일 만료 기준으로 계산
- 모든 가격 후보는 비클릭 참고 정보로 표시
- 첫 화면은 9개 제품으로 제한하고 검색·정렬·찜·상세 상태를 URL 또는 브라우저에 보존
- 상품 공유 URL과 오류 신고 문맥에 상품 ID·이름·현재 판매 경로를 포함
