...
| 판매 데이터 자동 운영 | 완료      | 일 1회 수집·엄격 검사·자동 커밋·OCI 앱 배포·운영 smoke                           |
...
- 검증된 구매 링크와 숨김 수는 매일 수집 결과와 30일 만료 기준으로 계산
- 쿠팡 구매 링크 0개
...
후보 데이터와 판매 근거는 매일 자동 갱신하지만, 배송비와 결제 단계 재고가 없는 후보는 비클릭 참고 정보다. 신선한 배송비 포함 가격을 확보하기 전까지 `가격 다시 확인`을 유지하는 것이 의도된 동작이다.
