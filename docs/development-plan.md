...
| 단계                  | 상태      | 완료 기준                                                                        |
| --------------------- | --------- | -------------------------------------------------------------------------------- |
| 데이터 정규화         | 완료      | 33개 canonical 제품, 빈 제목 제거, 중복 통합, 품질 상태 산출                     |
...
| 공식몰 링크 자동 운영 | 완료      | 일 1회 실응답 점검·엄격 검사·자동 커밋·OCI 앱 배포·운영 smoke                    |
| 운영 smoke            | 완료      | 공개 URL·정적 자산·보안 헤더·48시간 점검 신선도·전체 상품 링크를 시간당 2회 확인 |
...
- 모든 상품에 정리된 검색어를 제공하며 공식몰 링크는 매일 실응답 점검과 30일 만료 기준으로 계산
- 쿠팡 구매 링크 0개
...
2. 일반 소스 배포도 검증된 CI artifact와 attestation만 OCI에 반영하도록 확장한다.
3. 실패 알림을 운영자가 즉시 확인할 채널에 연결한다.
4. SSH ingress를 운영자 고정 IP로 제한할지 결정한다.

2026-07-31 네이버 쇼핑 API 종료로 자동 가격 수집을 제거했다. 매일 공식몰 링크만 점검하며 실패·만료 시 상품 검색으로 전환한다. 과거 가격 후보는 날짜를 유지한 비클릭 참고 정보다. 자세한 장애 원인과 현재 운영은 `price-sync.md`를 따른다.
