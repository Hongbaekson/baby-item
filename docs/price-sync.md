...
OCI 운영 서버에서는 가격 동기화 비밀값을 repo 내부 `.env`가 아니라 서버 전용 파일로 관리한다.

- 경로: `/etc/euni-baby-items/price-sync.env`
- 소유자: `ubuntu:ubuntu`
- 권한: `600`
- Git 추적 대상 아님
- 실행 시에만 `--env-file /etc/euni-baby-items/price-sync.env`로 일회성 컨테이너에 주입

현재 OCI에는 네이버 쇼핑 검색 API 키만 설정되어 있다. 쿠팡 키는 미설정 상태다.
...
docker run --rm \
  ...
  -w /app \
  node:20-alpine \
  node scripts/collect-naver-price-candidates.mjs
```

## 쿠팡 OpenAPI 판단

공식 문서 기준으로 쿠팡 `상품 조회 (승인불필요)` API는 일반 공개 상품 검색 API가 아니다.

- 호출 path는 `/v2/providers/seller_api/apis/api/v1/marketplace/seller-products/{sellerProductId}/partial`이다.
- 필요한 식별자는 `sellerProductId`이며, 이는 쿠팡에 상품 생성 완료 시 나오는 등록상품 ID다.
- 응답은 배송/반품지 등 판매자 상품 관리 정보를 조회하는 성격이다.
- `승인불필요`는 상품 정보 수정/조회 단계에서 별도 승인 절차가 필요 없다는 의미에 가깝고, API Key 발급 조건을 없애는 의미가 아니다.
- 쿠팡 OpenAPI Key 발급 문서는 WING 판매자 등록과 사업자 인증을 전제로 하며, 일반회원에게는 API Key 발급이 지원되지 않는다고 안내한다.

따라서 현재 앱의 용도인 “우리 목록의 일반 육아템을 쿠팡에서 검색해서 최저가/품절을 갱신”에는 이 seller API를 그대로 쓰기 어렵다. 쿠팡 후보 수집은 다음 중 하나가 준비되어야 안정적으로 진행할 수 있다.

- 쿠팡 판매자 계정으로 OpenAPI Key 발급
- 접근 가능한 쿠팡 파트너스/제휴 상품 검색 API 권한 확보
- 쿠팡 상품 URL 기반의 허용된 제휴/상품 피드 확보
- 쿠팡은 보조 후보로 두고, 1차 가격 후보는 네이버 쇼핑 검색 API로 수집
...
5. 쿠팡 API는 seller/제휴 권한 확보 후 후보 수집 스크립트 실행
