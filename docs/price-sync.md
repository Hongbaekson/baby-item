...
후보와 판매 링크 수는 매일 달라질 수 있으므로 문서에 고정하지 않는다. 최신 수치는 `data/data-quality-report.json`, GitHub Actions의 `validated-sale-data-*` artifact, `npm run links:check` 결과에서 확인한다. 대표 CTA는 가격 보장이 아니라 검증된 판매 경로이며, 최신 가격·배송비·재고는 네이버 또는 공식몰에서 다시 확인해야 한다.
