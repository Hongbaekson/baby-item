...
- 단축 URL을 실제로 열어 리다이렉트 목적지를 확인하면 제휴 클릭/통계에 영향이 생길 수 있어 별도 결정 후 진행한다.
...
- Security List 또는 NSG는 실제 공개 포트만 연다.
...
1. 단축 URL을 그대로 둘지, 최종 목적지 URL로 교체할지 결정
2. OCI에서 80 포트를 바로 열지, 8080으로 먼저 검증 후 80으로 전환할지 결정
3. SSH 접근을 특정 IP로 제한할지 결정
