...
- LLM/Hermes/Codex는 제품 후보 매칭과 이상치 검토에만 사용하고, 가격 숫자의 원천으로 사용하지 않는다.
- `보러가기` 버튼은 `bestOffer`가 있을 때만 검증된 최저가 링크로 대체한다.
- `bestOffer` 자동 반영 조건은 HTTPS URL, 품절 아님, 유효한 가격/배송비, 높은 제품 매칭 신뢰도다.
- API 키와 모델 키는 GitHub Secrets 또는 OCI 환경변수로만 관리하고 repo에 커밋하지 않는다.
