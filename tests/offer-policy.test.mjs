...
test("purchase-link evidence expires after thirty days", () => {
  const now = new Date("2026-07-26T00:00:00.000Z");
  assert.equal(
    isCurrentPurchaseEvidence("2026-06-27T00:00:00.000Z", now),
    true,
  );
  assert.equal(
    isCurrentPurchaseEvidence("2026-06-25T23:59:59.000Z", now),
    false,
  );
  assert.equal(isCurrentPurchaseEvidence(null, now), false);
});
